...
class CreateUserService:
  ...
  def execute(self, name: Type[str], email: Type[str], profile: Type[str]):
    
    verify_data = isinstance(name, str) and isinstance(profile, str or None) and isinstance(email, str)
    ...
    user = self.user_repository.create_user(name=name, email=email, profile=profile)
